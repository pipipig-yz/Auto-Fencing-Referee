...
class AthleteBaseline:
    ...
    def quality_check(self, side: str) -> None:
        ...
        if math.isnan(self.stance_width) or self.stance_width < config.BASELINE_STANCE_MIN_PX:
            logger.warning(
                "Baseline quality warning — %s athlete stance_width=%.1fpx is too small. "
                "Ankle keypoints may be overlapping in side-view. "
                "Lunge ratio detection will be disabled for this athlete.",
                side, self.stance_width if not math.isnan(self.stance_width) else -1,
            )
...
class AthleteTracker:
    ...
    def __init__(self):
        ...
        # Short rolling buffer for temporal angle smoothing (blur mitigation)
        self._angle_buf_L: deque[dict] = deque(maxlen=config.ANGLE_SMOOTH_WINDOW)
        self._angle_buf_R: deque[dict] = deque(maxlen=config.ANGLE_SMOOTH_WINDOW)
    ...
    def update(self, frame_idx: int, detections: list[PoseDetection]) -> FrameData:
        ...
        # Temporal smoothing — stabilise joint angles against motion-blur noise
        _smooth_angles(data.L, self._angle_buf_L)
        _smooth_angles(data.R, self._angle_buf_R)
        ...
        data.baseline_captured = self.baseline_L.captured or self.baseline_R.captured
        return data
    ...
    def _try_capture_baseline(self, data: FrameData) -> None:
        ...
        If only one side reaches stability, the other side's current angle
        buffer is used as its baseline (shared capture). This handles videos
        where one athlete's on-guard pose is less stable or one side is briefly
        occluded during the calibration window.
        """
        if data.L.detected:
        ...
        if not self.baseline_L.captured and _is_stable(self._still_angles_L):
            ...
            # Share with Right if Right hasn't been captured yet
            if not self.baseline_R.captured and self._still_angles_R:
                avg_R = _average_angles(list(self._still_angles_R))
                self.baseline_R.capture(avg_R)
                self.baseline_R.quality_check("Right")
                logger.warning("Right baseline not yet stable — shared from Left capture moment "
                               "(elbow=%.1f° front_knee=%.1f°)",
        ...
        if not self.baseline_R.captured and _is_stable(self._still_angles_R):
            ...
            logger.info("Baseline captured for RIGHT athlete: elbow=%.1f° front_knee=%.1f°",
                        self.baseline_R.elbow_angle, self.baseline_R.front_knee_angle)
            # Share with Left if Left hasn't been captured yet
            if not self.baseline_L.captured and self._still_angles_L:
                avg_L = _average_angles(list(self._still_angles_L))
                self.baseline_L.capture(avg_L)
                self.baseline_L.quality_check("Left")
                logger.warning("Left baseline not yet stable — shared from Right capture moment "
                               "(elbow=%.1f° front_knee=%.1f°)",
                               self.baseline_L.elbow_angle, self.baseline_L.front_knee_angle)


# Joint angles to smooth — excludes position fields used for velocity computation
# (com_x, front_ankle_x, rear_ankle_x) so that velocity estimates stay unaffected.
_SMOOTHED_ANGLES = {
    "elbow_angle", "shoulder_angle",
    "front_knee_angle", "rear_knee_angle",
    "inter_thigh_angle", "trunk_lean",
    "stance_width", "wrist_height",
}


def _smooth_angles(af: AthleteFrame, buf: deque) -> None:
    """Replace joint angles in af with a rolling mean over the buffer.

    Inspired by Lin et al. (MMSports'23): short temporal averaging over a
    window of ANGLE_SMOOTH_WINDOW frames reduces keypoint noise introduced
    by motion blur during fast limb movements, without perceptible lag at 30fps.
    """
    if not af.detected or not af.angles:
        buf.clear()
        return

    buf.append({k: af.angles[k] for k in _SMOOTHED_ANGLES if k in af.angles})

    if len(buf) < 2:
        return

    for key in _SMOOTHED_ANGLES:
        vals = [d[key] for d in buf if key in d and not math.isnan(d[key])]
        if vals:
            af.angles[key] = float(np.mean(vals))
