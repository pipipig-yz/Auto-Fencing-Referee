"""Scan all videos and save every frame where an athlete satisfies BOTH attack
conditions simultaneously:
    elbow_angle    > ATTACK_ELBOW_ABS    (150°)   — arm fully extended
    shoulder_angle > ATTACK_SHOULDER_ABS  (70°)   — arm raised away from body

Output: output/attack_scan/<video_stem>/<frame>_<L|R|LR>.jpg

Usage (from the code/ directory):
    python scripts/debug_attack_scan.py
    python scripts/debug_attack_scan.py --no-piste
"""
...
def _is_attack(af, bl):
    if not af.detected or not bl.captured:
        return False
    elbow    = af.angles.get("elbow_angle",    math.nan)
    shoulder = af.angles.get("shoulder_angle", math.nan)
    return (not math.isnan(elbow)    and elbow    > config.ATTACK_ELBOW_ABS
...
def _save_frame(frame, detections, frame_idx, fps, af_L, af_R, bl_L, bl_R,
    ...
    draw_top_label(img, f"ATTACK  {sides}", frame_idx, fps)
    draw_athlete_metrics(img, "L", af_L, bl_L)
    draw_athlete_metrics(img, "R", af_R, bl_R)

    cv2.imwrite(str(out_dir / f"{frame_idx:05d}_{sides}.jpg"), img)


def process_video(video_path: Path, use_piste: bool,
                  pose: PoseModule, piste: PisteDetector) -> int:
    out_dir = Path(config.OUTPUT_ROOT) / "attack_scan" / video_path.stem
    ...
    for frame_idx, frame in iter_frames(cap, target_fps=config.TARGET_FPS):
        ...
        trig_L = _is_attack(pose_data.L, tracker.baseline_L)
        trig_R = _is_attack(pose_data.R, tracker.baseline_R)
...
def main() -> None:
    ...
    for rel_path in VIDEOS:
        ...
        saved = process_video(video_path, not args.no_piste, pose, piste)
        out = Path(config.OUTPUT_ROOT) / "attack_scan" / video_path.stem
